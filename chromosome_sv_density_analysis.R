...
loadPileup <- function(dat.in, centromeres.in, repcov.in=NULL, window=11, smooth=T){
 ...
 colnames(dat)[1] <- "chr"
 cents_f <- read.table(centromeres.in, header=F, comment.char="")
 colnames(cents_f)[1] <- "chr"
 cents <- data.frame(chr=character(),
                     start=integer(),
                     end=integer(),
                     stringsAsFactors=FALSE) 
 for (CHROM in chrom_order) {
   cents <- rbind(cents, c(CHROM,
                           min(cents_f[cents_f$chr == CHROM, 2:3]),
                           max(cents_f[cents_f$chr == CHROM, 2:3])))
 }
...
generateCovPlotsPerChrom <- function(mat, 
 ...
 plotMiniCovSummary <- function(contigs, ymax=NULL){
  ...
  sapply(1:length(contigs), function(i){
   rect(xleft=Nmasks[which(Nmasks[, 1]==paste0("chr", contigs[i])), 2]+c(0, cumsum(chr.lengths[contigs]))[i], 
      xright=Nmasks[which(Nmasks[, 1]==paste0("chr", contigs[i])), 3]+c(0, cumsum(chr.lengths[contigs]))[i], 
      ybottom=0, ytop=ymax, 
      border=NA, col="gray80")
   sapply(1:nrow(Nmasks[which(Nmasks[, 1]==paste0("chr", contigs[i])), ]), function(k){
    if((Nmasks[which(Nmasks[, 1]==paste0("chr", contigs[i])), 3]-Nmasks[which(Nmasks[, 1]==paste0("chr", contigs[i])), 2])[k]>10000000){
     text(x=mean(c(Nmasks[which(Nmasks[, 1]==paste0("chr", contigs[i])), 2][k]+c(0, cumsum(chr.lengths[contigs]))[i], 
            Nmasks[which(Nmasks[, 1]==paste0("chr", contigs[i])), 3][k]+c(0, cumsum(chr.lengths[contigs]))[i])), 
...
## self-define args
SAMPLE_INVESTIGATED <- "IPF3353"
dat.in <- paste0(SAMPLE_INVESTIGATED, ".binned_counts_bySVTYPE.autosomes.bed")
centromeres.in <- "refs/h38.centromeres.bed"
repcov.in <- NULL
nmask.in <- "refs/h38.large_Nmasked_blocks.bed"
OUTDIR <- paste0(SAMPLE_INVESTIGATED, "_test_plots")
prefix <- SAMPLE_INVESTIGATED
svtypes.file <- "vcf_qc_refs/SV_colors.txt"
denovos.in <- NULL
...
meta.svtypes <- c("ALL", "DEL", "INS", "INV")
...
generateCovPlotsPerChrom(mat=lapply(chrom_order, function(k){return(as.data.frame(dat[which(dat$chr==k), c(1:3, which(colnames(dat)=="ALL"))]))}), 
             colors="gray10", 
             contigs.top=1:5, 
             contigs.middle=6:12, 
...
png(paste(OUTDIR, "/", prefix, ".SV_density_idiograms_bySVTYPE.png", sep=""), 
  height=800, width=2600, res=300, bg="white")
generateCovPlotsPerChrom(mat=lapply(chrom_order, function(k){return(as.data.frame(dat[which(dat$chr==k), c(1:3, which(colnames(dat) %in% c("DEL", "DUP", "INS")))]))}), 
